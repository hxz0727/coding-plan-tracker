...
| 事项 | 9/22 状态（前次） | 9/24 状态（最新） |
| --- | --- | --- |
| **Copilot 9/28 统一体验 + Balanced 默认** | 🔴 剩 6 天 | 🔴 **剩 4 天**（三端合一 + Chat 数据留存改「账号生命周期」；**退出统一体验 = 失去 github.com / Mobile 的 Copilot 访问**；代码审查默认 Lite→Balanced，想留 Lite 须提前在 repo/org **显式选择**——默认值会变，显式设置才被尊重） |
| **Copilot 10/1 新席位预付** | 🔴 剩 9 天 | 🔴 **剩 7 天**（**价格不变、仅收费时点提前**；已分配席位在计费周期开始时预付，**闲置席位同样收费、撤销不退费**；新客户另需先付费后开通） |
| **🆕 Copilot 按供应商列表价计费（9/22）** | — | 🔴 **本期最大治理变更**：Opus 5.5 / GPT-6 Sol / GPT-6 Luna 三款**均按供应商列表价以用量计费、不公示 premium-request 倍率**；**席位费买的是访问权而非 token**；**新模型默认自动启用**——管理员不作为即等于「全组织最贵前沿模型开着且被计量」；**Opus 5.5 与 Sol 止步 Pro+，仅 Luna 下探 Pro** |
| **🆕 Copilot 10/19 模型退役清单** | 🔴 10/19 一组三线退役（未列名） | 🔴 **10/19 剩约 25 天**：**Gemini 3.7 Flash、GPT-5.5、GPT-5.4、GPT-5.4 mini、GPT-5 mini、Grok 4.5**，覆盖 **Chat / inline edits / ask / agent / completions**（与 10/2 四模型切换、OpenAI 侧 10/14 GPT-5.5 退役分属不同批次，须分别审计 pin 模型） |
| **🆕 GPT-5.5 退役（OpenAI 侧）** | 🔴 拟 10/14（剩约 22 天） | 🔴 **拟 10/14（剩约 20 天）**：ChatGPT / Work / Codex **全档下线**，**API 不受影响**，工作区默认 / 自定义 Agent / 定时任务 / 脚本请迁至 **GPT-6 Sol 或 Astra** |
| **🆕 Claude Opus 5.5 发布（9/22）** | 🔥 Claude 5.2 灰度（未发布，无定价） | ✅ **已正式发布并成默认旗舰**：**\$4/\$20**（降 20%）、**cache read \$0.20（降 60%）**、Batch 5 折、Fast \$8/\$40、1M/128K、`claude-opus-5-5`；SWE-bench Pro 89.9%、Terminal-Bench 4.0 66.4%；**🔴 更正：Fable 5.2 最终未发布，灰度对象实为 Opus 5.5**；已入 **Claude Code v2.1.280 默认** + **Copilot 四档** |
| **🆕 Opus 5.5 订阅侧权益** | — | 🟢 **Pro / Max / Team 及席位制 Enterprise 的 5 小时用量上限取消**，并上线**限流重置按钮**；据 Anthropic，**Sonnet 5.5 与 Haiku 5.5 将于「数周内」发布** |
| **🆕 GPT-6 Sol / Luna 发布（9/22）** | — | ✅ **已发布，价格永久腰斩**：**Sol \$2/\$10**、**Luna \$0.10/\$0.50**（非促销、无到期日）；1.05M 上下文 / 128K 输出 / 922K 单次输入；**缓存读 90% 折扣且改 effort 不失效缓存**；`gpt-6-sol` / `gpt-6-luna`；**ChatGPT Work + Codex 灰度**（Plus/Pro/Business/Enterprise/Edu），Free/Go 可经桌面应用用 Luna |
| **Copilot 新注册重开** | 🟢 9/3 起重开 | 🟢 9/3 起重开（信用卡 / PayPal，账户审核加强） |
| **Copilot auto 模型选择「成本/质量」配置** | 🟢 9/14 上线 | 🟢 9/14 上线；HydraFusion 编排对外落地 |
| **Copilot Grok 4.7 上架** | 🟢 9/21 起五档上架 | 🟢 9/21 起五档上架（Pro/Pro+/Max/Business/Enterprise）；**usage-based billing + provider list pricing**；**Free 不含** |
| **火山方舟 2.5 折** | 🟢 剩 47 天（至 11/8） | 🟢 **剩 45 天**（Lite ¥9.9 / Pro ¥49.9 首两月，至 2026-11-08 23:59:59；名额有限） |
| **🆕 火山 deepseek-v4.1-flash 抵扣 5 折** | — | 🟢 **9/23 – 10/30（剩 36 天）**：在 Coding Plan 中抵扣系数**在现有基础上再享 5 折**（官方文档 9/23 口径） |
| **🆕 火山 Kimi-K2.8-Preview 6 折** | — | ⏰ **9/18 – 9/30（剩 6 天）**：Coding Plan 中可用量相当于 Agent Plan 6 折活动期 |
| **🆕 火山封号红线** | — | 🚨 **官方重申**：在非 AI 编程工具中使用 Coding Plan 的 Base URL / API Key **可能被判滥用/违规 → 订阅停用或账号封禁**；且必须走指定 Base URL |
| **GPT-5.6 Sol 促销（旧价）** | 🟢 剩 60 天（\$4/\$20） | ⚫ **已被 GPT-6 Sol（\$2/\$10 永久价）取代**；Devin 7 折至 10/3（剩 9 天）仍在 |
| **Gemini 3.8 Flash intro 价** | 🟢 剩 100 天（至 12/31） | 🟢 **剩 98 天**（至 12/31，2027 翻倍至 \$1.50/\$7.50） |
...
| **智谱 ZCode 合规风波** | 🔴 10/10 最后通牒 | 🟢 **已闭环**：v3.14.0 移除 RepoWiki 与仓库快照生成/上传路径、**代码 9/21 GitHub 开源（Apache-2.0）**、**信通院 + 绿盟核查确认阿里云 OSS 存储桶及数据对象已删除**、**承明科技撤回函件及主张**；⚠️ 但仍未说明受影响用户规模与历史数据是否曾被解密/访问，且开源仓库仅 2 个提交、无版本标签（**可验证性有限**）；**股价三日累跌逾 16%** |
| **智谱 Coding Plan 限量发售** | 🔴 2 月停售 / 7 月复售 15 倍增长 | 🔴 **维持供给风险**（2 月算力告罄停售、7 月复售超 15 倍增长、中报后两周 Coding 用户 +100 万）；**9/20 另上 ZCode 专属 Start Plan** |
| **智谱 GLM-5.2 下线（火山方舟）** | ✅ 8/31 14:00 已下线（更正） | ✅ **8/31 14:00 已下线**（自动路由至 GLM-5.3）；Z.ai 直连 API 仍列 5.2（\$1.40/\$4.40） |
| **智谱 GLM 夜间畅用** | 🟢 9/20 已截止 | 🟢 9/20 已截止（9/21 起恢复积分制：高峰 3× / 非高峰 1×） |
| **Z.ai 非高峰 1×** | ⏰ 约 8 天（至 9 月底） | ⏰ **约 6 天**（至 9 月底） |
| **OpenAI 终止向 Cursor 直供模型** | 🔴 拟 11/12（剩约 51 天） | 🔴 **拟 11/12（剩约 49 天）**：此后 Cursor 保留 Claude / Gemini / Grok、**失去 GPT**；**Cursor 的 Other Models 池以 GPT 为主者，该日用量画像将突变** |
| **OpenAI→Cursor / Cursor 定价** | — | 📌 **Cursor 现行价（9/22 核验）**：Free \$0 / Pro \$20 / Pro+ \$60 / Ultra \$200 / Teams 标准席位 \$40（年付 \$32）/ 高级席位 \$120（年付 \$96）/ Enterprise 报价制；**Auto 平价模式已于 8/24 终止**，Auto 按所选路由模型列表价计费，Teams/Enterprise 用第三方模型另有加价 |
| **DeepSeek V4.1 Flash 发布** | 🔥 9/10 已发布；V4.1 Pro 未发布 | 🔥 9/10 已发布；**V4 Pro 仍正常服务、9/14 路由未生效（9/11 撤回，维持）**；**V4.1 Pro 截至 9/24 仍未发布**（官方无日期/规模/定价/基准）；📊 **周用量 +483.62%、由第 9 升至第 2** |
| **双节前模型发布窗口** | 📌 10+ 款集中 9/21–10 月中 | 📌 **部分兑现**（Sol/Luna、Opus 5.5、Grok 4.7、FlashX、Qwen3.8-Omni-Flash、Step 5 Preview）；**待发**：**Sonnet 5.5 / Haiku 5.5（「数周内」）**、Gemini 4.0、Kimi K3.1（传 10 月）、Qwen4、GLM-5.5、Mimo V2.6、MiniMax M3.1、HY4 Pro、V4.1 Pro ⚠️ 9 月底前不宜锁定长期年付档 |
| **Anthropic IPO** | ⏳ S-1 截至 9/22 仍无公开；传由 10 月推迟至 11 月 | ⏳ **WSJ 9/18 确认由 10 月推迟至 11 月**（要带 Q3 财报而非预测；7 月底年化收入超 \$650 亿）；**9/24 更报道「推迟至 11 月中期选举之后」**；**公开 S-1 截至 9/24 仍未提交**；目标估值最高 **\$2 万亿**、募资最高 **\$1,000 亿**、私人轮投后 \$9,650 亿；MS + GS 主导、JPM + 花旗参与；**\$150 亿循环信贷**、英伟达基石最高 \$100 亿；竞争：**Astra 约占企业 AI 支出 13% > Fable 约 8%**；**SEC 规则：修订须在路演前 ≥15 天公开** |
| **Qwen3.8-Omni-Flash 发布** | 🟢 9/17 发布 | 🟢 9/17 发布：原生全模态、音频价降 98%+ |
| **Moonshot / Kimi 港股 IPO** | 🟢 9/3 秘密递表（\$50B / \$3B） | 🟢 9/3 秘密递表（\$50B / \$3B）；**K3 9/19 已上 Amazon Bedrock**；**K3.1 传 10 月**（Low/High/Max + 1M + Swarm） |
| **Stripe 收购 OpenRouter** | ⏳ 交割中 | ⏳ 交割中（路由中立性待观察） |
| **OpenAI IPO** | ⏳ 推迟至 2027 | ⏳ **推迟至 2027**（Altman 确认 2026 年不启动） |
...
## 一、近期主要动态（2026 年 6–8 月 + 9/1–9/24 落点）
...
- **9/24 落点（最新）**：🔥🟢 **Claude Opus 5.5 正式发布（9/22）**——Claude 5.5 家族首款、新任默认旗舰，**\$4/\$20**（降 20%）、**cache read \$0.20（降 60%）**、Batch 5 折、Fast \$8/\$40、1M/128K，`claude-opus-5-5`，已上 API + AWS + Google Cloud + Azure，并同日进入 **Claude Code v2.1.280 默认**与 **GitHub Copilot 四档**；SWE-bench Pro 89.9%、Terminal-Bench 4.0 66.4%，**以 Fable 5.1 约 40% 单价交付 Fable 级能力**；🔴 **更正：Fable 5.2 最终未发布，9/19–9/20 的「5.2 灰度」实为 Opus 5.5**；订阅侧**取消 Pro/Max/Team 的 5 小时用量上限**并上限流重置按钮，**Sonnet 5.5 / Haiku 5.5 称「数周内」**。🔥🟢 **OpenAI GPT-6 Sol / Luna（9/22）价格永久腰斩**——**Sol \$2/\$10、Luna \$0.10/\$0.50**（明确非促销、无到期日），1.05M 上下文 / 128K 输出 / 922K 单次输入，**缓存读 90% 折扣且改 effort 不失效缓存**，同日进 Copilot（Sol 止步 Pro+、Luna 下探 Pro）。🔴 **Copilot 三款前沿模型「默认自动开启 + 按供应商列表价计费、不公示倍率」**——席位费买访问权而非 token，固定人头成本旁出现无上限可变成分，本期最大治理/预算风险；配套 **10/19 退役清单明确**（Gemini 3.7 Flash / GPT-5.5 / GPT-5.4 / GPT-5.4 mini / GPT-5 mini / Grok 4.5，覆盖 Chat / inline edits / ask / agent / completions）。⏳ **Anthropic IPO：WSJ 9/18 确认由 10 月推迟至 11 月**（要带 Q3 财报；7 月底年化收入超 \$650 亿），**9/24 报道进一步称推迟至中期选举之后**，**公开 S-1 截至 9/24 仍未提交**；Q2 营收 \$115 亿、**首次录得调整后营业利润 \$5.59 亿**、2028 年指引 \$1,900–2,000 亿。🔴 **智谱 ZCode 合规风波闭环**（v3.14.0 移除 RepoWiki 与仓库快照上传路径、代码 9/21 GitHub 开源 Apache-2.0、信通院 + 绿盟核查确认阿里云 OSS 数据已删除、承明科技撤回函件），但**代价已现：股价三日累跌逾 16%**，且受影响用户规模与历史数据是否被解密/访问**仍未说明**。🟢 **火山方舟（9/23 官方文档）**：`deepseek-v4.1-flash` 在 Coding Plan 抵扣系数 **5 折至 10/30**、Kimi-K2.8-Preview 6 折至 9/30，并**重申非编程工具使用套餐 Base URL/Key 的封号红线**。🟢 **腾讯云 WorkBuddy 与 CodeBuddy 均 198 元/月**（9/23 深圳 AI 博览会现场口径）。📊 **用量侧：DeepSeek V4.1 Flash 周用量 +483.62%**（降价后由第 9 升至第 2）；📌 **倒计时：Copilot 9/28 剩 4 天、10/1 剩 7 天、10/19 退役剩约 25 天**。
