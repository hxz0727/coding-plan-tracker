...
> **最新更新：2026-09-21** ｜ 更新频率：每日 09:05（自动化）
...
| 事项 | 9/20 状态（前次） | 9/21 状态（最新） |
| --- | --- | --- |
| **Copilot 9/28 统一体验 + Balanced 默认** | 🔴 剩 8 天（官方 changelog 8/28 确认，静默涨价；数据终身留存） | 🔴 **剩 7 天**（官方 changelog 8/28 确认，静默涨价；数据终身留存） |
| **Copilot 10/1 新席位预付** | 🔴 剩 11 天（先付费后开通，撤销不退费） | 🔴 **剩 10 天**（先付费后开通，撤销不退费） |
...
| **Copilot 模型退役（一组）** | 🟢 10/2 四模型完成切换（付费、按 token 计费） | 🟢 **10/2 四模型切换** + 🔴 **新增 10/19 一组模型三线退役**（Chat / inline edits / code completion，9/18 changelog 确认；建议审计 pin 模型） |
| **GPT-6 Astra 发布** | 🟢 9/3 已发布，分阶开放中 | 🟢 9/3 已发布，分阶开放中 |
| **GPT-5.5 退役** | 🔴 拟 10/14 退役（剩约 24 天）：ChatGPT / Work / Codex 全档下线，API 不受影响，迁移至 GPT-5.6-Sol | 🔴 **拟 10/14 退役（剩约 23 天）**：ChatGPT / Work / Codex 全档下线，API 不受影响，迁移至 GPT-5.6-Sol |
| **火山方舟 2.5 折** | 🟢 剩 49 天（Lite ¥9.9 / Pro ¥49.9 首两月，至 11/8） | 🟢 **剩 48 天**（Lite ¥9.9 / Pro ¥49.9 首两月，至 11/8） |
| **GPT-5.6 Sol 促销** | 🟢 剩 62 天（$4/$20）；Devin 7 折至 10/3 仍在 | 🟢 **剩 61 天**（$4/$20）；Devin 7 折至 10/3 仍在 |
| **Gemini 3.8 Flash intro 价** | 🟢 剩 102 天（至 12/31，2027 翻倍 $1.50/$7.50） | 🟢 **剩 101 天**（至 12/31，2027 翻倍 $1.50/$7.50） |
| **智谱 GLM-5.3-Flash 5 折** | 🔴 已截止（9/10 起恢复刊例 $0.15/$0.50，国内 ¥0.8/¥2.8） | 🔴 **已截止**（9/10 起恢复刊例 $0.15/$0.50，国内 ¥0.8/¥2.8） |
| **智谱 GLM-5.3-FlashX 高速版** | 🟢 9/18 发布：200 tok/s 高速变体，国际 $0.37/$1.25、国内 ¥2/¥7 | 🟢 9/18 发布：200 tok/s 高速变体，国际 $0.37/$1.25、国内 ¥2/¥7 |
| **智谱 GLM-5.2 下线（火山方舟）** | ⏰ 剩 1 天（原误记 9/21 14:00 下线，仅留 5.3） | ✅ **8/31 14:00 已下线（🔴更正：官方 codingplan.org/火山方舟方舟 Coding Plan 确认 8/31 下线、自动路由至 5.3；9/21 为误记）**，仅留 5.3 |
| **智谱 GLM 夜间畅用** | 🟢 今日（9/20）截止（ZCode 内 0 额度窗口收尾；9/21 起恢复积分制） | 🟢 **9/20 已截止**（ZCode 内 0 额度窗口收尾；9/21 起恢复积分制，高峰 3×/非高峰 1×） |
| **Z.ai 非高峰 1×** | ⏰ 约 10 天（至 9 月底） | ⏰ **约 9 天**（至 9 月底） |
| **DeepSeek V4.1 Flash 发布** | 🔥 9/10 已发布；V4 Pro 路由未生效（维持）；V4.1 Pro 截至 9/19 仍未发布（qcode.cc tracker） | 🔥 9/10 已发布；V4 Pro 路由未生效（维持）；**V4.1 Pro 截至 9/21 仍未发布**（qcode.cc tracker 9/20 更新：Not released / One mention / Premise withdrawn） |
...
| **OpenAI 终止向 Cursor 直供模型** | 🔴 拟 11/12 终止（剩约 53 天） | 🔴 **拟 11/12 终止（剩约 52 天）** |
...
| **Claude Code 2.1.278** | 🟢 9/20 发布：Auto 模式 pre-flight 安全检查移至 Anthropic 服务器，不再计费 | 🟢 9/20 发布：Auto 模式 pre-flight 安全检查移至 Anthropic 服务器，不再计费 |
| **GitHub Copilot 代码审查重构 / budget increase GA** | — | 🟢 9/18 代码审查体验重构（清晰时间线 + 自动解决已处理建议 + 生成 commit message）；9/16 budget increase requests GA（Business/Enterprise 额度耗尽可即时申请加预算）；Copilot CLI 1.0.87 增强 prompt recall / session resume / MCP |
| **OpenAI Codex CLI 0.155.1** | — | 🟢 9/18 发布：新本地 TUI 默认关闭 reasoning summaries，修复第三方 OpenAI-compatible provider 被拒；0.156.0 alpha 已至 alpha.7 |
| **Anthropic IPO** | ⏳ 9 月下旬窗口已开启（现 9/20），S-1 截至 9/20 晨仍无公开（EDGAR 仅 6/1 机密 S-1）；维持 9 月下旬披露/10 月中旬路演/11 月上市、$2T、Nvidia 基石最高 $10B | ⏳ **9 月下旬/10 月初窗口（现 9/21），S-1 截至 9/21 仍无公开**（MEXC/winappnet 9/21 复检 EDGAR 仅 6/1 机密 S-1；部分来源 this-info 现称 S-1 early Oct）；维持 9 月下旬披露/10 月中旬路演/11 月上市、$2T、Nasdaq、$15B 循环信贷、Nvidia 基石最高 $10B |
