...
> **最新更新：2026-08-06** ｜ 更新频率：每周一、四 09:05（自动化）
> 数据来源：各厂商官方定价页 / 官方文档 / 官方博客 + 第三方价格追踪（截至 2026-08-06 交叉验证）

---

## ⭐ 本期新增动态（2026-08-05 → 08-06）

### 1. 🔴 重要澄清：火山方舟有**两个** 2.5 折活动——套餐价到 **11/8**、GLM-5.2 抵扣确为 **8/8** 到期（本期头条）

此前多期报告（含上期）称火山方舟套餐 2.5 折「6/8–8/8 截止」，本期核查**官方活动文档**后确认口径有误，特此修正：

| 来源 | 口径 | 判定 |
|---|---|---|
| **火山引擎官方活动文档**（docs.volcengine.com/docs/82379/2525063） | **2026/6/8 – 2026/11/8** | ✅ **以此为准** |
| 火山引擎开发者社区文章 | 「即日起至 2026 年 8 月 7 日」 | ⚠️ 单篇推广文档期，非活动总档期 |
| 第三方博客 | 「延至 8/27」 | ❌ 无官方依据 |

- **实际含义**：**套餐价** 2.5 折窗口长达 5 个月，**8 月 8 日并非套餐价最后机会**（GLM-5.2 抵扣 2.5 折确实 8/8 到期，详见下条），此前「截止前抢购」的建议属误导。
- **但仍有真实约束**：官方明确「**名额有限，先到先得**」——活动可能因名额耗尽提前结束，这才是真正的不确定性。
- **不变的规则**：每账号**最多享两个月** 2.5 折，第三个月起恢复原价；优惠资格在新购/续费/升配间共享，用过不补发。
- **另一个独立活动**：GLM-5.2 抵扣系数 2.5 折为 **6/10 18:00 – 8/8 23:59**，这个 8/8 是真的，但它是「模型扣额打折」不是「套餐价格打折」。**两个活动此前被混为一谈。**

> **结论：下期 8/8 只需关注 GLM-5.2 抵扣折扣是否到期，套餐 2.5 折本身还有 3 个月。**

### 2. 火山方舟 Agent Plan 完整档位曝光——业界首个「Agent 套餐包」

上期仅一句话带过，本期补齐全貌。这是**与 Coding Plan 并列的独立产品线**，计量单位为 **AFP（Agent 燃料值）**，把多模态模型与 Harness 工具打包销售。

**个人版**

| 档位 | 刊例价 | 限时特惠 | 额度 | 关键权益 |
|---|---|---|---|---|
| Small | ¥40/月 | **¥9.90/月** | 20,000 AFP | 体验版本，仅供测试；不支持生视频模型 |
| Medium | ¥200/月 | **¥49.90/月** | 5× Small | **免费赠 ArkClaw 轻量版**；支持 Kimi K3 |
| Large | ¥500/月 | — | 12.5× Small | 支持 Seedance 2.0 + Kimi K3 |
| Max | ¥1,000/月 | — | 25× Small | 顶配旗舰额度，生产级应用 |
...
- **与 Coding Plan 的差异**：Coding Plan 只有文本编程模型；Agent Plan **原生覆盖文本 / 代码 / 图像 / 视频全模态**（Doubao-Seed、Seedream 5.0 lite、Seedance 2.0），并附带 **Harness 能力**（免费联网搜索额度 + Embedding 记忆）。
- **成本优势（官方测算）**：同等预算下用 Agent Plan 调 DeepSeek V4 系列，比后付费 API 最高省 **80%+**；V4 Flash 折合 **1.9 折**，V4 Pro 在限时 2.5 折后再打 7 折，**每月最多省 ¥850**。
- **典型场景对比**：构建轻量短视频网站（前后端代码 + Logo + 配图 + 短视频），按后付费调用等量 V4 Pro/V4 Flash/Seedream/Seedance 约需 **¥709/月**，Agent Plan 可大幅压缩。
- **限额规则差异**：视觉模型**无 5 小时/周限额**，仅受日额度与月额度约束；语音模型与 Harness **无 5 小时/周限额**，仅受月额度约束。
- **超额后付费**：开启后额度耗尽自动切换按量计费，**无需改 Base URL / API Key / 模型名**；额度刷新后自动切回套餐抵扣。
- **邀请返利**：每邀请一位好友下单，获其订单金额 **5% 代金券（上不封顶）**，好友首订额外 **9.5 折**。
- 接入工具：Claude Code、OpenClaw、TRAE 等可同时调用模型与 Harness。

### 3. Coding Plan 品类一年生死簿：从 9 家 28 款到「基本只剩一家」（8/4 深度复盘）

本期最具信息密度的一条行业叙事，梳理出完整的品类兴衰曲线：

| 时间 | 事件 |
|---|---|
| 2025-02 | Anthropic 发布 Claude 3.7 Sonnet + Claude Code，4 月推出 Max Plan（\$100/\$200），**模式原型诞生** |
| 2025-09 | 智谱发布 GLM Coding Plan，**「Coding Plan」这个品类名由智谱首创**，最低 ¥20/月（Anthropic 的 1/7 价格、3× 用量） |
| 2026-01 | GLM-4.7 上线成为国产可用性分水岭，**算力扛不住，智谱限售至原来 20%** |
| 2026-02-12 | GLM-5 发布，Coding Plan 涨价 30%+、取消首购优惠，**上线当天售罄** |
| 2026-02 下旬 | 阿里云百炼 **¥7.9/月** 入场（聚合千问/GLM/Kimi/MiniMax） |
| 2026-03 | 腾讯云 ¥7.9 起、火山方舟 ¥8.9、百度千帆跟进；**9 家平台 28 款套餐，价格战顶点** |
| 2026-03 | 阿里 Lite 套餐下架；腾讯套餐售罄不再补货 |
| 2026-05 | 业内定论：**低价 Coding Plan 时代实质性终结**（从出现到消失不到 3 个月） |
| 2026-07-19 | **Kimi K3 发布 48 小时后暂停 C 端新用户订阅**（K3 单次加载近 3TB 显存，至少 64 张 H800，硬件投入超千万） |
| 2026-07-30 | 智谱改积分制 + 涨价 130%–261% |
| 2026-08 | **市面上仍在正常售卖 Coding Plan 的，基本只剩智谱与火山方舟** |

**核心悖论（本期最值得记住的一句）**：
> Coding Plan 的吸引力来自模型能力——模型越强，开发者越愿付费；但模型越强，推理成本越高。**开发者付固定月费，平台扛上涨成本，两条线方向相反。**

- 云厂商 ¥7.9 的账**从来就算不过来**——他们买的是开发者入口，不是这 7.9 元收入。但入口成本在飞涨：2025 年一个编程任务调 10 次模型，2026 年复杂任务调 50 次是常态。
- **DeepSeek 走了第三条路**：提供地板价 API，但**坚决不做 Coding Plan**。「订阅制是平台替用户扛用量波动风险；DeepSeek 说这个风险我不扛，我把单价压到最低，你自己管账单。」
- 当前各家状态：**京东云**已停 Coding Plan 新购；**阿里云百炼** Pro 版每日 9:30 限量抢；**智谱**涨价并停了老套餐自动续费；**腾讯云**已下线；**Kimi** 仍为「预约订阅」。

### 4. 阿里云百炼四计划体系全貌 —— 此前只记录了一半

上期只覆盖 Coding Plan 与 Token Plan，实际百炼有 **4 套并行的订阅/优惠机制**，且**存在互斥红线**（详见 3.3 节）：

| 计划 | 计量方式 | 关键数字 |
|---|---|---|
| **Token Plan 个人版** | Credits，5h + 7 天双窗口 | Lite ¥39（原¥60）/ Standard ¥139（原¥180）/ Pro ¥499（原¥600） |
| **Token Plan 团队版** | Credits，按席位、自然月重置 | 标准 ¥198/席 / 高级 ¥698/席 / 尊享 ¥1,398/席 |
| **Coding Plan** | **按调用次数**（非 Token） | 仅剩 Pro ¥200/月（9 万次/月）；Lite 已下架 |
| **Night Plan** | 夜间时段折扣 | 23:00–07:00，**低至 0.2 折**，可与其他计划叠加 |
| **AI 通用节省计划** | 承诺月消费换阶梯折扣 | 最高 **5.3 折**；入门型 10 抵 20 / 50 抵 100 / 250 抵 500 |

- ⚠️ **重大红线（此前未记录）**：**Coding Plan 与 Token Plan 权益互斥，同一账号不可同时订阅**；两者 API Key 与按量计费 Key **三方互不相通**。
- ⚠️ 两者均**禁止非交互式场景**（自动化脚本、API 后端服务、批量数据处理），违规可能封号。
- Coding Plan 专属接入：Key 以 `sk-sp-` 开头，Base URL 为 `https://coding.dashscope.aliyuncs.com/v1`。

### 5. 新增厂商收录：OpenCode（Go / Zen）—— 绕开国内限购的「平替」路线

在国内 Coding Plan 集体限购/停售的背景下，**OpenCode Go** 成为社区热议的替代方案，本期正式收录（详见 3.12 节）。

- **本体完全免费开源**（MIT）：**16 万+ GitHub Stars、900 位贡献者、月活开发者 750 万**，是 MiMoCode 等多款产品的技术底座。
- **OpenCode Go**：**首月 \$5，之后 \$10/月**，一次订阅打包 15 款主流开源/国产模型，**不用抢购、不限名额、支持信用卡/PayPal**。
- **模型月度请求额度**（差异极大，是选型关键）：
...
- **额度差达 264×**（Grok 4.5 的 120 次 vs V4 Flash 的 31,650 次）——**\$10 买的不是「随便用」，而是「便宜模型随便用」**。
- 还支持**复用已有订阅**：可用 GitHub 账号登录调用自己的 Copilot 额度，或用 OpenAI 账号调用 ChatGPT Plus/Pro 额度。

### 6. Qwen3.8 发布（8/3）：国际价仅为 Opus 5 的 40% / 24%

- **规格**：2.4T 总参 / **95B 激活** / **1M 上下文**，支持视觉理解，基于 Qwen 3.5 架构。
- **定价**：
...
- 官方口径：国际价格仅为 **Opus 5 的 40%（输入）与 24%（输出）**。
- **榜单**：Arena 综合**全球第二**（仅次于 Anthropic Claude 系列）；**CodeArena 全球第四**。
- **能力**：可从空文件夹出发，**自主完成十数天级真实项目交付，全程无人干预**（测试案例为从零构建自进化 harness 项目 oh-my-cli）。
- **开源计划**：Qwen3.8-Max 预计下周开源权重，同时开源 **Qwen3.8-27B**。
- 同日阿里发布 Agent 产品 **「千问办公」（QwenWork）** 公测——业内首款同时支持桌面端/云端/企业协同 Agent 的产品。

### 7. 8/4 全球降价潮全景：不是所有厂商都在降

上期只记录了 OpenAI，本期补全同期三家动作，可见**策略分化**：

| 厂商 | 动作 | 性质 |
|---|---|---|
| **OpenAI** | GPT-5.6 Luna **降价 80%**（\$1→\$0.20 输入，\$6→\$1.20 输出）、Terra 降 20% | 中低端降价 |
| **OpenAI** | 旗舰 **5.6 Sol 维持原价，Fast 模式反而涨价** | 高端**逆势提价** |
| **Google** | 推出更便宜的 **Gemini 3.6 Flash** | 新增低价档 |
| **Anthropic** | 用能力更强的 **Claude 5.0 替换低价位的 Opus 4.8，价格分文未动** | **以能力换价格**（等效降价） |

- 对比锚点：3 月发布的 GPT-5.4 为 \$2.5/\$15，**不到 4 个月，同级前沿模型输入价从 \$2.5 降到 \$0.20（-92%）**。
- Terra 降价后 \$2/百万 token，**已低于 Kimi K3 的 \$3**。
- **中国模型是导火索**：Kimi K3 与 DeepSeek V4 被多方点名为逼降主因。

### 8. 券商与政策：Token 经济正式进入政府文件

- **海通国际（8/5）**：模型竞争从**参数规模转向后训练与系统工程**——V4-Flash 未扩参数，靠后训练 + Harness 强化任务规划/工具调用/错误恢复。未来效果取决于「**模型 + Harness + 工具 + 数据**」整体组合。投资上看好云与推理基础设施，**对模型层保持谨慎**（价格战削弱单一 API 的稀缺性和定价权）。
- **华泰证券（8/5）**：DS V4-Flash 混合价 **\$0.06/百万 Token**、平均任务成本 **\$0.03**，较 Luna 分别低 **65% / 57%**。中国模型形成**「强能力（K3，57 分）+ 高性价比（V4-Flash，50 分）」两条路线**。
- **长江证券（8/5）**：Anthropic ARR 约 **\$741 亿**，OpenAI 7 月底 ARR 约 **\$600 亿**，**两家合计已超 \$1,300 亿且增长斜率未放缓**——Coding 场景是主要贡献。
- **政策（北京市）**：《关于加快智能体引领发展的若干措施》明确提出**「鼓励发展 Token（词元）经济」**、支持以 **OPC（一人公司）** 为代表的创新创业新模式。**这是 Token 经济首次进入省级政府正式文件。**
- **OpenRouter 最新周榜**：**DeepSeek V4 Flash 调用量第一（7.22T tokens）**；**Top 10 中 8 款为中国模型**（DeepSeek、小米、腾讯、智谱、MiniMax、阶跃、月之暗面）。
- **杰文斯悖论正在生效**：模型降价虽降低单次任务算力消耗，但扩大了用户数、调用频率与可经济部署的场景，**带动 Token 总消耗加速增长**。
...
- **Coding Plan 品类退潮**：从 2026-03 的 9 家 28 款，退到 8 月「基本只剩智谱 + 火山方舟」。腾讯云（4/22 下线）、百度千帆（7/13 停售）、京东云（停新购）、阿里（Lite 下架、Pro 限量）、Kimi（暂停订阅）先后退出或收缩。
...
> 2026 年国内出现明显的 **「Coding Plan → Token Plan」转型潮**，并在 7–8 月演变为**品类退潮**：从 3 月的 9 家 28 款，退到 8 月「基本只剩智谱 + 火山方舟」在正常售卖。腾讯云（4/22）、百度千帆（7/13）已完成切换，京东云停新购，阿里云百炼双轨并行且 Coding Plan 持续收缩，Kimi 自 7/19 起暂停订阅至今。
...
- **✅ 本期修正：2.5 折活动档期为 2026/6/8 – 2026/11/8**（官方活动文档口径），此前报告的「8/8 截止」有误。约束条件是「**名额有限，先到先得**」而非日期。
...
- **2.5 折活动**：Small ¥40→¥9.9、Medium ¥200→¥49.9，档期 **2026/6/8 – 2026/11/8**，规则与 Coding Plan 相同（每账号最多两个月）。
...
- **4/22 起页面 404，Coding Plan 全面下线**，转向 **Token Plan（Credits 计量）**。此前曾**一个月内两次调价**。
...
| GitHub Copilot | ✅ Free | Business/Enterprise 促销额度**至 8 月底** | github.com / 企业协议 | ⚠️ **Pro/Pro+/Student 自 4/20 起暂停新注册** |
...
| 阿里云百炼 | ✅ 免费领 7,000 万 Tokens | Token Plan 个人版活动价约 65–83 折 | 阿里云 | ⚠️ **Coding Plan Pro 每日 9:30 限量；与 Token Plan 互斥** |
| 阿里 Qoder CN | **✅ 体验版（14 天 Pro + 300 Credits）** | 首次试用每人限一账号 | **Qoder.com.cn**（支付宝） | 否（多试用账号会被封停） |
| 火山方舟 Coding Plan | ❌（送 2,500 万 Tokens） | **首两月 2.5 折，档期 6/8–11/8** | 火山引擎 | ⚠️ 5 月起限购；名额有限先到先得 |
| **火山方舟 Agent Plan** | ❌（送 2,500 万 Tokens） | **Small ¥9.9 / Medium ¥49.9（6/8–11/8）**；邀请返 5% | 火山引擎 | ⚠️ 名额有限先到先得 |
| 小米 MiMo | **✅ MiMoCode + MiMo-V2.5 限时永久免费** | 首购 88 折 / 包年 88 折 / 首开自动续费 77 折 | platform.xiaomimimo.com | 否（需实名，不退款） |
| MiniMax | ❌ | 连续包年立省 2 个月 | platform.minimaxi.com | 否 |
| **Kimi** | ✅ Adagio 免费档 | 年付约 8 折 | kimi.com/code | ❌ **C 端新用户订阅自 7/19 起暂停，仅可「预约」，恢复日期未披露** |
...
0. **Coding Plan 作为一个品类正在退潮**。从 2026-03 的 9 家 28 款，到 8 月「基本只剩智谱 + 火山方舟」在正常售卖。**根本矛盾无解**：模型越强 → 开发者越愿付固定月费 → 推理成本越高 → 平台亏损越大。Kimi 是最极端案例——**做出了最好的产品，却因为越卖越亏而只能不卖**。
...
3. **⚠️ 也要警惕「截止日焦虑营销」**：本期发现火山方舟**套餐价** 2.5 折被广泛误传为「8/8 截止」，实际官方档期到 **11/8**（GLM-5.2 抵扣系数 2.5 折确为 8/8 到期，是另一个独立活动）。**遇到「最后机会」类信息，务必回查厂商官方活动文档页。**
...
| 个人轻度 | 百度千帆 Mini ¥4.9 / 火山方舟 Small ¥9.9 / Trae Lite \$3 |
...
| **需要生图/生视频** | ⭐ **火山方舟 Agent Plan Medium ¥49.9**（多模态 + Harness 打包，比后付费省 80%+） |
...
- **8/8** GLM-5.2 抵扣系数 2.5 折到期（**注意：火山套餐 2.5 折本身到 11/8，两者不同**）；**8/15** 智谱包年 7 折截止；**8/17** Claude 旧版 Workbench 退役；**8/31** Claude Sonnet 5 限时价到期（+50%）；**8 月底** Copilot Business/Enterprise 促销额度到期；**9 月底** Z.ai 非高峰 1× 扣额优惠到期；**11/8** 火山方舟 Agent/Coding Plan 2.5 折到期。
