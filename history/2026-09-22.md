...
> **最新更新：2026-09-22** ｜ 更新频率：每日 09:05（自动化）
...
| 事项 | 9/21 状态（前次） | 9/22 状态（最新） |
| --- | --- | --- |
| **Copilot 9/28 统一体验 + Balanced 默认** | 🔴 剩 7 天（官方 changelog 8/28 确认，静默涨价；数据终身留存） | 🔴 **剩 6 天**（官方 changelog 8/28 确认；三端合一 + Chat 数据留存改「账号生命周期」；**退出统一体验 = 失去 github.com/Mobile Copilot 访问**；代码审查默认 Lite→Balanced，想留 Lite 须提前在 repo/org 显式选择） |
| **Copilot 10/1 新席位预付** | 🔴 剩 10 天（先付费后开通，撤销不退费） | 🔴 **剩 9 天**（**价格不变、仅收费时点提前**；已分配席位周期开始时预付，**闲置席位同样收费**；新客户另需先付费后开通） |
| **Copilot 新注册重开** | 🟢 9/3 起重开（信用卡/PayPal） | 🟢 9/3 起重开（信用卡/PayPal，账户审核加强） |
| **Copilot auto 模型选择「成本/质量」配置** | 🟢 9/14 上线；HydraFusion 编排对外落地 | 🟢 9/14 上线；HydraFusion 编排对外落地 |
| **🆕 Copilot Grok 4.7 上架** | — | 🟢 **9/21 起五档上架**（Pro/Pro+/Max/Business/Enterprise；八处选择器）；**usage-based billing + provider list pricing**（1 credit=\$0.01）；**Free 不含**；Business/Enterprise 默认自动启用，须用 model policy 管控 |
| **Copilot 模型退役（一组）** | 🟢 10/2 四模型完成切换 + 🔴 新增 10/19 一组模型三线退役 | 🟢 **10/2 四模型切换** + 🔴 **10/19 一组三线退役**（Chat / inline edits / code completion；官方已给出推荐替代模型，Business/Enterprise 可用 model policy 预启用） |
| **GPT-6 Astra 发布** | 🟢 9/3 已发布，分阶开放中 | 🟢 9/3 已发布，分阶开放中；Astra for Law 专项模型已出（99.9% 美国判例库） |
| **GPT-5.5 退役** | 🔴 拟 10/14 退役（剩约 23 天）：ChatGPT / Work / Codex 全档下线，API 不受影响，迁移至 GPT-5.6-Sol | 🔴 **拟 10/14 退役（剩约 22 天）**：ChatGPT / Work / Codex **全档下线、无遗留访问**，API 不受影响，迁移至 **GPT-5.6 Sol 或 GPT-6 Astra** |
| **🆕 Anthropic Claude 5.2 灰度发布** | — | 🔥 **9/19–9/20 起后台静默路由**（Fable 5.1→5.2、Opus 5→Opus 5.2/Next、Sonnet 5→5.2）；⚠️ **官方未发布**（无 model ID / 卡 / 定价 / 日期）；提升幅度 Opus 5.2 > Sonnet 5.2 > Fable 5.2；Fable 5.2 靠 slow thinking 长程逻辑压过 Astra，但更慢更贵 |
| **火山方舟 2.5 折** | 🟢 剩 48 天（Lite ¥9.9 / Pro ¥49.9 首两月，至 11/8） | 🟢 **剩 47 天**（Lite ¥9.9 / Pro ¥49.9 首两月，至 11/8） |
| **GPT-5.6 Sol 促销** | 🟢 剩 61 天（$4/$20）；Devin 7 折至 10/3 仍在 | 🟢 **剩 60 天**（$4/$20）；Devin 7 折至 10/3 仍在 |
| **Gemini 3.8 Flash intro 价** | 🟢 剩 101 天（至 12/31，2027 翻倍 $1.50/$7.50） | 🟢 **剩 100 天**（至 12/31，2027 翻倍 $1.50/$7.50） |
| **智谱 GLM-5.3-Flash 5 折** | 🔴 已截止（9/10 起恢复刊例 $0.15/$0.50，国内 ¥0.8/¥2.8） | 🔴 **已截止**（9/10 起恢复刊例 $0.15/$0.50，国内 ¥0.8/¥2.8） |
| **智谱 GLM-5.3-FlashX 高速版** | 🟢 9/18 发布：200 tok/s 高速变体，国际 $0.37/$1.25、国内 ¥2/¥7 | 🟢 9/18 发布：200 tok/s 高速变体，国际 \$0.37/\$1.25、国内 ¥2/¥7；**Coding Plan 内需申请开通**（非全档默认可用，9/20 实测） |
| **Grok 4.7**（xAI，9/21） | **\$2.00** | \$0.50 | **\$6.00** | 500K | ≤200K prompt；>200K 为 \$4/\$1/\$12；**与 Grok 4.6 同价**；CursorBench 4.0 46.3%、DeepSWE v1.1 71.0%；Copilot/OpenRouter 同价；⚠️ 单位任务 token 消耗高 |
| **🆕 智谱 Coding Plan 限量发售 + ZCode Start Plan** | — | 🔴 **2 月因算力告罄被迫停售 Coding Plan；7 月复售 → 超 15 倍销量增长**；中报后两周 Coding 用户 +100 万；**9/20 上线 ZCode 专属付费体验档 Start Plan**（疑似 Coding Plan 外新服务）；⚠️ 合规风波未平（10/10 最后通牒） |
| **智谱 GLM-5.2 下线（火山方舟）** | ✅ 8/31 14:00 已下线（🔴更正：官方确认 8/31 下线、自动路由至 5.3） | ✅ **8/31 14:00 已下线**（官方确认，自动路由至 GLM-5.3）；Z.ai 直连 API 仍列 5.2（\$1.40/\$4.40）；部分聚合池 id 临时路由至 5.3 至 9/30 |
| **智谱 GLM 夜间畅用** | 🟢 9/20 已截止（ZCode 内 0 额度窗口收尾；9/21 起恢复积分制，高峰 3×/非高峰 1×） | 🟢 **9/20 已截止**（ZCode 内 0 额度窗口收尾；9/21 起恢复积分制，高峰 3×/非高峰 1×） |
| **Z.ai 非高峰 1×** | ⏰ 约 9 天（至 9 月底） | ⏰ **约 8 天**（至 9 月底） |
| **DeepSeek V4.1 Flash 发布** | 🔥 9/10 已发布；V4 Pro 路由未生效（维持）；V4.1 Pro 截至 9/21 仍未发布 | 🔥 9/10 已发布；V4 Pro 路由未生效（维持）；**V4.1 Pro 截至 9/22 仍未发布**——The Information 爆料「**2 万亿参数（后续 8 万亿）、预计 10 月中下旬发布、押注华为昇腾**」（非官方） |
| **🆕 双节前模型发布窗口** | — | 📌 **10+ 款集中在 9/21–10 月中**：GPT-6 Sol/Terra/Luna、Opus 5.2 + Fable 5.2、Gemini 4.0 Pro/Flash、Step 5 Preview（已上线）、Kimi K3.1、Qwen4、Mimo V2.6、GLM-5.5、MiniMax M3.1、HY4 Pro、V4.1 Pro ⚠️ 9 月底前不宜锁定长期年付档 |
| **DeepSeek 科创板 IPO 筹备** | 🟢 已聘中信证券，计划年内启动；按 $710 亿投前估值融资、拟任首位 CFO | 🟢 已聘中信证券，计划年内启动；按 \$710 亿投前估值融资、拟任首位 CFO |
| **国家超算 ¥9.9 促销** | ⚫ 618 限时已结束；现基础版活动价 ¥30/月 | ⚫ 618 限时已结束；现基础版活动价 ¥30/月 |
| **OpenAI 终止向 Cursor 直供模型** | 🔴 拟 11/12 终止（剩约 52 天） | 🔴 **拟 11/12 终止（剩约 51 天）**；⚠️ **Cursor 的 Other Models 池以 GPT 为主者，该日用量画像将突变**，勿据此日前的池结构做升级决策 |
| **Kimi K3 登陆 AWS Bedrock** | 🟢 9/19 上线：月之暗面 K3 正式入 Amazon Bedrock，兑现 9/3 递表时云托管路线 | 🟢 9/19 上线：月之暗面 K3 正式入 Amazon Bedrock，兑现 9/3 递表时云托管路线 |
| **Claude Code 2.1.278** | 🟢 9/20 发布：Auto 模式 pre-flight 安全检查移至 Anthropic 服务器，不再计费 | 🟢 9/19 发布（9/20 扩散）：Auto 模式安全检查移至 Anthropic 服务器、**不再计费**；⚠️ **若企业 LLM 网关剥离 `safeguards` 字段 / 重写 `anthropic-beta` 头 / 丢弃 `safeguard_results` 响应键，会静默回退到计费分类器**——须用 `/status` Auto mode server 行核验，网关应「原样透传」 |
| **GitHub Copilot 代码审查重构 / budget increase GA** | 🟢 9/18 代码审查体验重构；9/16 budget increase requests GA；Copilot CLI 1.0.87 | 🟢 9/18 代码审查体验重构（跨推送追踪 findings + 显式解决原因 + 智能 commit message）；9/16 budget increase requests GA；Copilot CLI 1.0.87 |
| **OpenAI Codex CLI 0.155.1** | 🟢 9/18 发布：新本地 TUI 默认关闭 reasoning summaries，修复第三方 provider 被拒 | 🟢 9/18 发布（0.155.1）：默认关闭 reasoning summaries；**0.155.0** 另引入实验性 `/voice`、实时 reasoning 摘要、Agent 任务管理、Touch ID 验证、**Amazon Bedrock 凭据支持** |
| **🆕 GPT-5.3-Codex-Spark（Cerebras）** | — | 🟢 **OpenAI × Cerebras 研究预览**：面向实时软件开发，Cerebras 硬件上 **>1000 tok/s**；**ChatGPT Pro 用户经 Codex app / CLI / VS Code 扩展可用** |
| **Anthropic IPO** | ⏳ 9 月下旬/10 月初窗口（现 9/21），S-1 截至 9/21 仍无公开 | ⏳ **S-1 截至 9/22 晨仍无公开**（anthropicvaluation.com 持续轮询 EDGAR 确认仅 6/1 保密草案；**SEC 规定修订须在路演前 ≥15 天公开**）；**上市地 Nasdaq、$2T、募资或超 \$1000 亿（超 SpaceX 成史上最大）、英伟达基石最高 \$100 亿**；⚠️ **据新浪财经 9/21，上市已由 10 月推迟至 11 月**；Polymarket：10 月约 56%、年底前约 85% |
| **Anthropic cache-read 降价** | 🟢 9 月初已落地：cache-read 价格下调 75% | 🟢 9 月初已落地：cache-read 价格下调 75%（长会话每消息成本显著低于标价） |
| **🆕 Anthropic 定价规则细节** | — | 📌 **Claude 4.7 及以后使用新 tokenizer，同文本约多产生 30% token**（标价低不等于账单低）；**cache 读取为输入价 1/10**；**Fable 5 在 Max 及 premium 席位含于套餐（每周限额最多一半可用），Pro 与 Team 标准席位须走 usage credits**，且 Fable 抽取周限额快于其他 Claude 模型 |
...
| **OpenAI IPO** | ⏳ 推迟至 2027 | ⏳ 推迟至 2027（Altman 确认今年不上市） |
...
- 🚨 **ZCode「代码上传」合规风波（9/18–9/21，订阅者风险评估新增）**：9/18 智谱就 ZCode 仓库百科异常上传公开道歉，但**当日内网仍监测到上传行为**；客户承明科技已下达**10/10 前书面答复最后通牒**（要求物理删除证明、操作日志、私钥保管与跨境传输说明，保留举报与诉讼权利）。**ZCode 9/19 更新「修复仓库百科异常上传问题」**。同期 **9/20 智谱悄悄上线 ZCode 专属付费体验档 «Start Plan»**（疑似 Coding Plan 之外的新服务）。**企业采购建议：在合规审计结论出来前，避免将含私有代码/密钥的仓库接入 ZCode，改用 API + 自建 Agent 或已审计的第三方工具。**
